...
import { ref, reactive, computed, provide, nextTick, isProxy, watchEffect, watch, unref, onMounted, onUnmounted } from 'vue'
...
onMounted(() => {
  ...
  window.addEventListener('keydown', (event) => closeCalendar(event));
});

onUnmounted(() => {
  window.removeEventListener('click', clickOutsideSelect);
  window.removeEventListener('keydown', (event) => closeCalendar(event));
})
  ...
  if (s && e && !off) {
    if (date.isSame(s, 'date')) {
      classes = e.isAfter(s, 'date')
        ? 'bg-primary text-white font-bold rounded-custom start-active disabled:cursor-not-allowed'
        : 'bg-primary text-white font-bold rounded-custom end-active disabled:cursor-not-allowed'
      if (s.isSame(e, 'date')) {
        classes = `bg-primary text-white font-bold rounded-full disabled:cursor-not-allowed`
      }
    }
...
const betweenRangeClasses = (date) => {
  ...
  if (s && e) {
    if (date.isSame(s, 'date')) {
      if (e.isBefore(s)) {
        classes += ` rounded-custom end-active inset-0`
      }
      if (s.isBefore(e)) {
        classes += ` rounded-custom start-active inset-0`
      }
...
function clickOutsideSelect(event) {
  function clickOutsideRecursively(element) {
    if (pickerContainer.value === element) {
      return false;
    }
...
<template>
  <div v-if="!props.noInput" id="vtd" ref="pickerContainer" class="relative w-full" @click="openCalendar">
    ...
    >
      <div v-if="!props.disabled && open" class="relative z-50">
        <div class="absolute z-50 top-full sm:mt-2.5" :class="getAbsoluteParentClass(open)">
          ...
          >
            <div
              class="vtd-datepicker static sm:relative bg-background dark:bg-card w-full sm:rounded-lg sm:shadow-sm border-0 sm:border border-black/[.1] px-3 py-3 sm:px-4 sm:py-4 dark:border-vtd-secondary-700/[1]"
            ...
            >
              <div class="flex flex-wrap lg:flex-nowrap">
                ...
                <div class="relative flex flex-wrap sm:flex-nowrap p-1 w-full">
                  ...
                  <div
                    class="relative w-full md:w-1/2 lg:w-70"
                    :class="{
                  ...
                  <div
                    v-if="asRange() && !props.asSingle"
                    class="relative w-full md:w-1/2 lg:w-70 overflow-hidden mt-3 sm:mt-0 sm:ml-2"
                  >
